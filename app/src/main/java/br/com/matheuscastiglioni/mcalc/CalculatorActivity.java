...
public class CalculatorActivity extends AppCompatActivity {
    ...
    @OnClick({
        ...
        R.id.btnCalculator_equals,
        R.id.btnCalculator_multiply,
        R.id.btnCalculator_percentage,
        R.id.btnCalculator_subtract,
        R.id.btnCalculator_sum
    })
    public void operate(Button btn) {
        if (this.left == null) {
            this.left = new Number(Integer.parseInt(this.textViewResult.getText().toString()));
            CalculatorView.clear(this.textViewResult);
        }
        if (this.right == null) {
            this.right = new Number(Integer.parseInt(this.textViewResult.getText().toString()));
            CalculatorView.clear(this.textViewResult);
        }
    }
